...
<body>
    {{-- <div class="logo">
        <img src="{{ public_path('public/LOGOBAWAH.png') }}" width="200" alt="Your Logo">
    </div> --}}
    ...
    <table style="width: 100%">
        ...
        <tr>
            ...
            <td rowspan="0">{{ $data['room_type'] }}</td>
            @endif
            <td>{{ $data['amount'] }}</td>
            <td>Rp. {{ number_format($data['price'], 0, ',', '.') }}</td>
            <td>Rp. {{ number_format($data['amount'] * $data['price'], 0, ',', '.') }}</td>
            @php
            $prevRoomType = $data['room_type'];
            $totalSum += $data['amount'] * $data['price'];
            @endphp
        </tr>
        @endforeach
        <tr>
    ...
    @endif
    <p>Special Request:</p>
    <p>{!! nl2br(e($reservation->special_req ?? '-')) !!}</p>
